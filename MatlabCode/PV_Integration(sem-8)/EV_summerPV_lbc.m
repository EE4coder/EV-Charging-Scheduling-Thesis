...
%........ BEV=>20%=>'1'____PHEV-40=>30%=>'2'____PHEV-30=>50%=>'3'[LBC]
    for i=1:nEV
       if i<=round(nEV*20/100)
           Array_BEV_40PHEV_30PHEV(i)=1;
       elseif i>round(nEV*20/100) && i<=round(nEV*50/100)
           Array_BEV_40PHEV_30PHEV(i)=2;
       elseif i>round(nEV*50/100) && i<=nEV
...
DM = normrnd(55,10,[1,nEV]);  % Daily Mileage following Normal Distribution 
                             % Mean=55, S.D.=10, No of Random Variables supposed to be generated=10
FTD = normrnd(18,8.41,[1,nEV]);  % 1st Trip Distance following Normal Distribution

Tin = normrnd(9,1.2,[1,nEV]);  % Arrival Time following Normal Distribution
Tout = normrnd(19,1.2,[1,nEV]); % Departure Time following Normal Distribution

while max(Tout)>24    % Tout shouldn't go beyond 24hrs
    Tout = normrnd(22,1.2,[1,nEV]);  %if goes, again generates Random Variable
end

rate_USEP_MWh1 = [104.37	96.8475	88.29	82.725	79.4025	78.7425	78.8175	78.3375	75.135	75.06	78.7275	78.7575	79.335	84.855	96.7125	69.635	75.9	100.39	104.715	84.905	82.18	75.88	73.195	69.435	64.46	69.645	69.68	46.15	50.795	42.655	100.155	107.1075	107.1075	109.2075	113.775	119.0925	131.64	156.255	229.8525	229.86	229.8225	229.875	177.2775	138.6075	113.7375	107.04	104.6175	104.55];
